...
cp "${CACHE_DIR}/rhel-server-7.8-x86_64-kvm.qcow2" "${VM_DIR}/${CLUSTER_NAME}-lb.qcow2" || \
    err "Copying '${VM_DIR}/rhel-server-7.8-x86_64-kvm.qcow2' to '${VM_DIR}/${CLUSTER_NAME}-lb.qcow2' failed"; ok
...
virt-customize -a "${VM_DIR}/${CLUSTER_NAME}-lb.qcow2" \
    ...
    --sm-register --sm-attach auto --install haproxy --install bind-utils \
